...
Author: Duncan McNab
File Encryptor and Decryptor using AES (Fernet) with PBKDF2 key derivation

Usage:
    To encrypt a file:
        python secure_encryption.py <file_path> --encrypt
    
    To decrypt a file:
        python secure_encryption.py <file_path.encrypted> --decrypt
...
class SecureEncryption:
    ...
    def encrypt_data(self, data: str, password: str) -> tuple: #Encrypt the provided data using Fernet (AES)
        key = self.generate_key(password)
        f = Fernet(key)
        encrypted_data = f.encrypt(data.encode())
        return encrypted_data, self.salt

    def decrypt_data(self, encrypted_data: bytes, password: str, salt: bytes) -> str: # Decrypt the encrypted data using the provided password and salt
        self.salt = salt
        key = self.generate_key(password)
        f = Fernet(key)
